...
class EnhancedParameterFusion:
    ...
    #         meta_file = self.results_dir / "smplestx_results" / "camera_meta.json"
    #         if not meta_file.exists():
    #             raise FileNotFoundError("CRITICAL: camera_meta.json not found")
    ...
    def _load_smplestx_params(self) -> Dict:
        # Load first available SMPLest-X params file
        for param_file in self.results_dir.glob('smplestx_results/*/person_*/smplx_params_*.json'):
            with open(param_file, 'r') as f:
                params = json.load(f)
            print(f"   ✅ SMPLest-X: Loaded from {param_file.relative_to(self.results_dir)}")

            # Camera meta is run-level: smplestx_results/camera_meta.json
            meta_file = self.results_dir / "smplestx_results" / "camera_meta.json"
            if not meta_file.exists():
                raise FileNotFoundError(f"CRITICAL: camera_meta.json not found at {meta_file}")

            with open(meta_file, "r") as f:
                self.camera_meta = json.load(f)
            print(f"   ✅ Camera metadata loaded from {meta_file.relative_to(self.results_dir)}")
    ...
    def load_mano_mean_poses(self):
        ...
        mano_model_dirs = [
            '/home/partner/3d_whole_body_pipeline/adapters/mano_data',
            'adapters/mano',
            'mano/models'
        ]
    ...
    def create_fused_parameters(self, smplx_params: Dict, wilor_params: Dict, emoca_params: Dict) -> Dict:
        """
        CORRECTED FUSION: Applies coordinate transformations to align WiLoR with SMPLest-X.
        Integrates EMOCA's jaw pose and applies the calculated scale/translation.
        """
        print("\n🔧 Creating fused parameters (with coordinate transformation)...")
        ...
        # --- HAND FUSION WITH COORDINATE TRANSFORMATION ---
        # Extract hand poses and apply the coordinate transformation
        ...
        # Apply coordinate transformation to the translation vector
        # Transformation: P_smplx = S * P_wilor + T
        # This aligns WiLoR's coordinate system with SMPLest-X
        print(f"   📐 Applying coordinate transformation:")
        print(f"      Scale factor: {self.scale_factor:.6f}")
        print(f"      Translation: {self.translation}")
        
        # Adjust translation to account for coordinate system alignment
        fused['translation'] = fused['translation'] + self.translation
        print(f"   ✅ Adjusted translation: {fused['translation']}")
        ...
        print("   ✅ Hands: WiLoR with coordinate alignment applied")
        return fused
    ...
    def create_render_gallery(self, individual_params: Dict, original_mesh: np.ndarray, enhanced_mesh: np.ndarray):
        ...
        if not hasattr(self, 'camera_meta'):
            print("   ⚠️  Camera metadata not loaded.")
            return
        
        cam_params = {
            'focal': np.array(self.camera_meta['focal_length']),
            'princpt': np.array(self.camera_meta['principal_point'])
        }
...
"""
    ...
    def save_results(self, original_params: Dict, fused_params: Dict, original_mesh: np.ndarray, enhanced_mesh: np.ndarray):
        ...
        print("\n🔍 Validating outputs...")
        assert (self.fusion_dir / 'fused_parameters.json').exists(), "❌ fused_parameters.json missing"
        assert (self.fusion_dir / 'enhanced_mesh.npy').exists(), "❌ enhanced_mesh.npy missing"
        
        renders = list(self.gallery_dir.glob('*.png'))
        assert len(renders) >= 4, f"❌ Expected ≥4 renders, got {len(renders)}"
        
        required_renders = ['1_input.png', '4_smplestx_overlay.png', '6_fusion_overlay.png']
        for render in required_renders:
            assert (self.gallery_dir / render).exists(), f"❌ {render} missing"
        
        print("✅ All critical outputs validated")
